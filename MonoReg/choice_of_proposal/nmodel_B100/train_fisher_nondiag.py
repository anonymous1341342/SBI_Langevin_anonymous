...
def main(task_id):
    ...
    checkpoint = torch.load(f'nmodel_init_nondiag/checkpoint_task{task_id}_trainsize{training_size}.pth', map_location=device)
    ...
    if not os.path.exists('nmodel_fisher_nondiag'):
        os.makedirs('nmodel_fisher_nondiag')

    torch.save({
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'bias_lastlayer': bias_lastlayer
    }, f'nmodel_fisher_nondiag/checkpoint_task{task_id}_trainsize{training_size}.pth')
