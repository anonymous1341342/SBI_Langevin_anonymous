...
def main(task_id):
    ...
    checkpoint = torch.load(f'nmodel_fisher/checkpoint_task{task_id}_trainsize100000.pth', map_location=device)
    ...
    os.makedirs(f'sample_res_nmodel_fisher', exist_ok=True)

    pd.DataFrame( pred_ys_ann_repar3.numpy() ).to_csv(f'sample_res_nmodel_fisher/pred_ys_ann_task{task_id}.csv', index=False, header=False)
    np.save(f'sample_res_nmodel_fisher/theta_init_task{task_id}.npy', theta_init.cpu().numpy())
    np.save(f'sample_res_nmodel_fisher/theta_r1_task{task_id}.npy', theta_r1.cpu().numpy())
