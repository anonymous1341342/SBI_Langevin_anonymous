...
f'DebRegModel_fisher_crossterm/model_task{task_id}.pth',
map_location=device)
...
os.makedirs('sample_res_DebReg_fisher_cross', exist_ok=True)

pd.DataFrame( pred_ys_ann_repar3.numpy() ).to_csv(f'sample_res_DebReg_fisher_cross/pred_ys_ann_task{task_id}.csv', index=False, header=False)
np.save(f'sample_res_DebReg_fisher_cross/theta_init_task{task_id}.npy', theta_init.cpu().numpy())
np.save(f'sample_res_DebReg_fisher_cross/theta_r1_task{task_id}.npy', theta_r1.cpu().numpy())
