# Copied and revised from mono_BP folder
# For Langevin sampling, keep more than the last samples in the chain
from source_monoBP_single import *
import os
...
# Reg_sample_size = int(config["Reg_sample_size"])
# Reg_obs_size = int(config["Reg_obs_size"])

# theta_r0 = pd.read_csv(f"res_r0/theta_r0_task{task_id}.csv")
# theta_r0 = torch.tensor(theta_r0.values, dtype = torch.float32).contiguous()
...
checkpoint = torch.load(f'model_single_fisher_nondiag/checkpoint_task{task_id}_trainsize{int(1e6)}.pth', map_location=device)
...
DebRegModel = torch.load(
f'DebRegModel_fisher_crossterm_nondiag/model_task{task_id}.pth',
map_location=device)

def NScore_DebReged(theta, x): # the input x is "N-data"
    return model.cal_penalty(theta, x).sum(dim = 1) - DebRegModel(theta) * obs_size
...
def repar_draw_post_vec_annealing3(skip, annealing_set, lower, upper, x_obs, theta_init, epis = 0.001, S = 100):
    ...
    for id_ann in range(annealing_set.shape[0]):
        ...
        # if id_ann == annealing_set.shape[0] - 1: # draw more samples in the last run
        #     S = S * 3
        for i in range(S):
...
# generate initial samples, using precond samples without inflation
...
sample_size = 1000
theta_init = sample_trunc_fast(prop_mean, prop_cov, lower, upper, sample_size)
...
pred_ys_ann_repar3 = psi_grids @ torch.linalg.inv(A) @ theta_r1.T

# Ensure the output directory exists
os.makedirs('sample_res_DebReg_fisher_cross_nondiag', exist_ok=True)

pd.DataFrame( pred_ys_ann_repar3.numpy() ).to_csv(f'sample_res_DebReg_fisher_cross_nondiag/pred_ys_ann_task{task_id}.csv', index=False, header=False)
np.save(f'sample_res_DebReg_fisher_cross_nondiag/theta_init_task{task_id}.npy', theta_init.cpu().numpy())
np.save(f'sample_res_DebReg_fisher_cross_nondiag/theta_r1_task{task_id}.npy', theta_r1.cpu().numpy())
