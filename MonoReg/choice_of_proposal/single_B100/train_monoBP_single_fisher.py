...
def main(task_id):
    ...
    if not path_theta.exists():
        ...
        val_theta_r0, val_data_r0 = gen_ref_distinct_theta(mean_theta, std_theta, lower, upper, sample_size + 100)
    ...
    # ============= Extra Data ============== #
    path_theta_extra = Path(f'ref_R/theta_r0_extra_task{task_id}.npy')
    path_data_extra = Path(f'ref_R/data_r0_extra_task{task_id}.npy')
    path_val_theta_extra = Path(f'ref_R/val_theta_r0_extra_task{task_id}.npy')
    path_val_data_extra = Path(f'ref_R/val_data_r0_extra_task{task_id}.npy')
    path_theta_extra.parent.mkdir(parents=True, exist_ok=True) # ensure parent folder exists

    if not path_theta_extra.exists():
        theta_r0_extra, data_r0_extra = gen_ref(mean_theta, std_theta, lower, upper, extra_obs_size, extra_sample_size + 100)
    ...
    # remove nan in training data
    bad_mask = torch.isinf(theta_r0_extra).any(dim=1)
    theta_r0_extra = theta_r0_extra[~bad_mask]
    data_r0_extra = data_r0_extra[~bad_mask]
    theta_r0_extra = theta_r0_extra[:extra_sample_size]
    data_r0_extra = data_r0_extra[:extra_sample_size]

    # remove nan in validation data
