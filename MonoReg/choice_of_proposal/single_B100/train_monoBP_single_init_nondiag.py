...
def main(task_id):
    ...
    # ======= ref_S ====== #
    path_theta = Path(f'ref_S_nondiag/theta_r0_task{task_id}.npy')
    path_data = Path(f'ref_S_nondiag/data_r0_task{task_id}.npy')
    path_val_theta = Path(f'ref_S_nondiag/val_theta_r0_task{task_id}.npy')
    path_val_data = Path(f'ref_S_nondiag/val_data_r0_task{task_id}.npy')
    ...
    os.makedirs('model_single_init_nondiag', exist_ok=True)

    torch.save({
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'bias_lastlayer': bias_lastlayer
    }, f'model_single_init_nondiag/checkpoint_task{task_id}_trainsize{training_size}.pth')
