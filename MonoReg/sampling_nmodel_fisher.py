...
def main(task_id, simu_budget):
    ...
    sample_size = 1000
    theta_r0, data_r0 = gen_ref(mean_theta, std_theta, lower, upper, 1, sample_size)
    ...
    os.makedirs('sample_res_nmodel_fisher', exist_ok=True)

    pd.DataFrame( pred_ys_ann_repar3.numpy() ).to_csv(f'sample_res_nmodel_fisher/pred_ys_ann_task{task_id}_trainsize{training_size}.csv', index=False, header=False)
    np.save(f'sample_res_nmodel_fisher/theta_init_task{task_id}_trainsize{training_size}.npy', theta_init.cpu().numpy())
    np.save(f'sample_res_nmodel_fisher/theta_r1_task{task_id}_trainsize{training_size}.npy', theta_r1.cpu().numpy())
