...
def main(task_id, simu_budget):
    ...
    theta_r0 = np.load(f'training_data/theta_r0_task{task_id}.npy', mmap_mode='r')[:sample_size]
    data_r0  = np.load(f'training_data/data_r0_task{task_id}.npy',  mmap_mode='r')[:sample_size]
    ...
    if sched:
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=sched_patience, min_lr=min_lr)
        print(f"Scheduler is used, with patience = {sched_patience}, min_lr = {min_lr}")
    ...
    os.makedirs('nmodel_fisher', exist_ok=True)
    torch.save({
