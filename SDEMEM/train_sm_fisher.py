...
num_epochs = 6 # config["num_epochs"]
learning_rate = 2.5e-5 # config["learning_rate"]
...
extra_obs_size = 200 # int(config["extra_obs_size"])
...
lam_fisher = 0.01 # config["lam_fisher"]
...
def main(task_id):
    ...
    if not (path_theta.exists() and path_x.exists()):
        ...
        step = max(1, int(extra_ref_size / 10))
        kept = 0
    ...
    # x_r0_extra = (x_r0_extra - mean_x.repeat(1, extra_obs_size)) / std_x.repeat(1, extra_obs_size)
