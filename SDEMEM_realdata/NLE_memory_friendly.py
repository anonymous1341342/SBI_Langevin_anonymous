...
prior_mean = torch.tensor([5, 1, 3, -1, -1, -5, 0.5, 0, -0.8, -0.8, -0.8, -0.8], dtype = torch.float32)
prior_std = torch.tensor([1, 1, 1, 1, 1, 2, 1, 1, 0.5, 0.5, 0.5, 0.5], dtype = torch.float32)
# prior_std = torch.tensor([1, 1, 1, 1, 1, 2, 1, 1, 1, 1, 1, 1], dtype = torch.float32)
...
batch_size = 2000 # int(config["batch_size"]) 

ref_size = int(5e7) # int(config["ref_size"]) 
            ...
            for batch_idx, start in enumerate(range(0, n_train, batch_size), start=1):
                ...
                if batch_idx % 1000 == 0 or batch_idx == num_train_batches:
                    print(
                        ...
                        f"current loss = {loss.item():.6f} | time = {time.time() - time1:.2f}s",
                        flush=True,
                    )
