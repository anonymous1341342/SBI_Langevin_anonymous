...
def train_deb(model, optimizer, dataloader, val_dataloader, g, g1, num_epochs, scheduler, early_stop_patience, return_best_model = False):
    ...
    step_lr_per_update = (
        ...
        and isinstance(scheduler, torch.optim.lr_scheduler.StepLR)
        and scheduler.step_size > num_epochs
    )

    if step_lr_per_update:
        print(
            f"StepLR step_size {scheduler.step_size} > num_epochs {num_epochs}; "
            "stepping scheduler on optimizer updates instead of epochs"
        )
        ...
        for batch_sample in dataloader:
            ...
            if step_lr_per_update:
                ...
                for param_group in optimizer.param_groups:
                    param_group["lr"] = max(param_group["lr"], min_step_lr)
